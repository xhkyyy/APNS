...
public class PushNotificationManager {
	...
	public static PushedNotification getPushedNotificationInstance(
			Device device, Payload payload, int identifier) {
		return new PushedNotification(device, payload, identifier);
	}

	public void createNewSocket() throws SocketException, KeystoreException,
		...
		while (counter < pns.size()) {
			try {
				...
				for (; counter < pns.size(); counter++) {
					...
					bytes = getMessage(push.getDevice().getToken(),
							push.getPayload(), push.getIdentifier(), push);
	...
	private int processedResponses(boolean failRetry) {
		...
		/**
		 * public boolean isSuccessful() { if (!transmissionCompleted) return
		 * false; if (response == null) return true; if
		 * (!response.isValidErrorMessage()) return true; return false; }
		 */
